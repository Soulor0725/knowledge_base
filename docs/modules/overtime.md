...
| 类型 | 值 | 时间范围 | 计算规则 |
|------|-----|----------|----------|
| 平时加班 | weekday | 结束时间 18:30-23:59（>=2026-07-27）；19:00-23:59（<2026-07-27） | 统一从 18:30（新）/ 19:00（旧）开始计算 |
| 周末加班 | weekend | 08:30-23:59 | 从 start_time 开始，自动扣除 12:00-14:00 午餐 |

### 时长计算
- **平时加班**：根据记录日期使用不同基准时间（新旧规则并存）：
  - **>= 2026-07-27**：从 18:30 开始计算
  - **< 2026-07-27**：从 19:00 开始计算（历史数据不受影响）
...
| 400 | 平时加班结束时间需在 18:30-23:59 范围内（新规则，>=2026-07-27） |
| 400 | 平时加班结束时间需在 19:00-23:59 范围内（旧规则，<2026-07-27） |
| 400 | 周末加班时间范围为 08:30-23:59 |
| 400 | 该日期已存在加班记录 |
| 400 | 结束时间必须晚于开始时间 |

## 相关文件
- `routes/overtime.py` - 路由实现 (360行)
