...
### 🟡 SVG viewBox 尺寸不足导致内容溢出

- **Bug #18：分类月度趋势折线末端分类标签显示不全**
  - **版本**：v2.6.4
  - **现象**：「分类月度趋势 (Top 6)」图表右侧分类名被裁切，无法看到完整分类名
  - **根因**：SVG viewBox 宽度 700，绘图区 `plotW = chartW - padL - padR = 700 - 60 - 20 = 620`，末端标签定在 `x = padL + plotW = 680`，仅剩 20px 显示空间。中文分类名（如"餐饮美食"约 40px）必然溢出被裁切
  - **修复**：
    ```javascript
    // 前
    const chartW = 700, chartH = 260, padL = 60, padR = 20, padT = 30, padB = 50;
    // 后
    const chartW = 860, chartH = 260, padL = 60, padR = 160, padT = 30, padB = 50;
    // plotW 从 620 增至 640，末端标签 x=700，右侧留出 154px 显示空间
    ```
  - **教训**：
    - SVG 内嵌文本标签时，viewBox 宽度 = 绘图区 + 左侧 padding + **标签实际需要的右侧空间**
    - 末端标签场景：`padR` 必须 ≥ 最长标签宽度 + 6px 间距
    - 验证公式：`padL + plotW + 6 < chartW - maxLabelWidth`
    - 搜索关键词：`分类显示不全`、`标签溢出`、`viewBox 裁切` → 先算 padR 是否够大
    - **新增检查清单项**：新增 SVG 图表带末端标签时，`padR` ≥ 150px（中文 4 字标签）

### 🟡 暗色背景下的字体可读性

- **Bug #19：消费统计图表字体过小**
  - **版本**：v2.6.4
  - **现象**：消费统计 5 个图表区域（月度消费趋势、月度分类支出、分类月度趋势、分类支出占比、分类占比月度趋势）字体普遍 9-11px，在暗色背景小字号下难以阅读
  - **根因**：SVG 内部 `font-size="10"`/`"11"` 和 HTML `font-size:11px`/`14px` 设置过小
  - **修复**：
    - 区块标题（5 个）：`14px` → `16px`
    - Y 轴网格标签：`10px` → `12px`
    - X 轴月份标签：`11px` → `12px`
    - 柱状图/折线数值标签：`9-10px` → `12px`
    - 图例分类名：`11px` → `12px`
  - **教训**：
    - 暗色背景（`rgba(30,41,59,0.8)`）下，SVG 内部字体**最小** 12px，HTML 区块标题**最小** 16px
    - SVG `font-size` 使用绝对值（无单位属性），与 CSS `font-size` 数值差异大，需逐一核对
...
- **版本**：v2.6.8
- **现象**：销售报表页面同时包含规格汇总统计和订单来源（销售人统计）两个独立功能，页面内容过多，用户体验不佳
- **根因**：初期将所有统计功能集中在一个页面，未考虑功能模块的独立性
- **修复**：
  - 后端：新增独立 API 端点 `GET /api/kiwi-order-source`，支持年份筛选，只返回销售人统计数据
  - 前端：侧边栏新增「订单来源」入口，独立页面展示
    - 统计卡片：来源数/总数量/总金额
    - 来源卡片：按销售人分组，展示数量/金额/规格明细
  - 销售报表页面移除订单来源部分，只保留规格汇总统计
- **教训**：
  - 功能模块拆分原则：当一个页面包含多个独立功能时，应考虑拆分为独立模块
  - 拆分标准：功能独立性强、数据来源相同但展示维度不同、用户使用场景分离
  - API 设计：拆分后的模块应有独立的 API 端点，避免单个 API 返回过多冗余数据
  - 搜索关键词：`页面过长`、`功能臃肿` → 考虑模块拆分
